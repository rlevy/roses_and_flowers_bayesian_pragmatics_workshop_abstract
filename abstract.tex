...
%% Abstract
% We describe a previously unreported informativeness implicature, in
% which a coordinate phrase of the form "x and X", with X a
% superordinate category and x a specific member of that category, is
% pragmatically strengthened to be interpreted as "x and other X".  We
% present corpus evidence demonstrating that this implicature is
% widespread in English over a range of noun types, and that the
% pattern is historically stable.  The fundamental difficulty
% presented by this pattern for formal analysis is that the literal
% meaning of a complete utterance involving "x and X" is equivalent to
% the meaning of the utterance which would be obtained if only "x"
% were used, so that the proper division of pragmatic labor needs to
% occur between "x and X" and "x" alone.  We present two possible
% models within the framework of rational speech-act theory which
% achieve this division of pragmatic labor, one based on lexical
% uncertainty and the other based on an underlying probabilistic
% semantics.  
